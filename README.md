...
See also [Help:Citation tools - English Wikipedia](https://en.wikipedia.org/wiki/Help:Citation_tools) for more tools like this one. The [Diberri Template builder](https://citation-template-filling.toolforge.org/cgi-bin/index.cgi) is a similar but unrelated tool that in addition to citation templates can be used to generate Chemboxes with PubChem compound IDs and Drugboxes with [DrugBank](https://drugbank.com/) IDs. However, this tool fills only a few fields for Chemboxes; PubChem compound IDs cannot be used to generate Drugboxes; and compounds with DrugBank IDs are limited mostly to marketed drugs—a small selection of possible compounds. Drugbox generation using DrugBank IDs also seems to be broken currently and [has been for a while](https://web.archive.org/web/20200712040837/https://citation-template-filling.toolforge.org/cgi-bin/index.cgi?type=drugbank_id&id=DB00328). Hence the need for an alternative tool without these limitations.
